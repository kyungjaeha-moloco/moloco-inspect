...
역할:
- orchestrator가 product app 내부 폴더 구조를 직접 몰라도 preview 실행에 필요한 경로를 얻도록 합니다
...
- vite config path
- tsconfig path
- e2e helper script path

예시:

```ts
type MTProductRuntimeConfig = {
  productId: string;
  productFilePrefix: string;
  productSourcePrefix: string;
  appRelativePath: string;
  sourceAppRoot: string;
  worktreeAppRoot: string;
...
export type MTProductAdapter = {
  ...
  isProductFile(relativePath: string): boolean;
  isProductSourceFile(relativePath: string): boolean;
...
4. runtime config를 통해 orchestrator의 product-specific path를 줄이기
...
- [`/Users/kyungjae.ha/Documents/moloco-inspect/tooling/preview-kit/src/config.js`](/Users/kyungjae.ha/Documents/moloco-inspect/tooling/preview-kit/src/config.js)
...
까지는 연결된 상태입니다. 특히 preview bootstrap, capture, route verify, copy verify, runtime config는 이제 adapter contract 뒤로 올라왔습니다.

...
//
// Molly operational metrics — collection + aggregation.
//
// Collection:
//   recordEvent(type, payload) — in-memory ring buffer (last 2000) + file
//   append (orchestrator/state/molly-metrics-YYYY-MM-DD.ndjson). Async
//   write — call latency impact ~0.
//
// Aggregation:
//   getMetrics(window='1h'|'24h'|'7d') — merges memory ring + file and
//   aggregates 9 metrics.
//
// Event types:
//   - 'lib_call'       — result of molly-classifier / chat / status / prd-analyzer /
//                        plan-emitter / lifecycle call
//   - 'intake_result'  — final kind from /api/intake
//   - 'intake_error'   — /api/intake 500 response category
//
// Common fields on every event: t (ms), type, surface?
...
export function recordEvent(type, payload = {}) {
  ...
  // file append (best-effort, async — a write failure leaves the memory ring intact)
  try {
...
/**
 * Merges the memory ring with the most recent N day files and returns
 * all events after the cutoff.
 *
 * Exported — molly-cost.js also performs cost aggregation on the same events stream.
 */
export function loadEvents(cutoff, opts = {}) {
  const fromRing = ring.filter((e) => e.t >= cutoff);
  // Also load from files — for a 24h+ window, include yesterday's file too
  ...
  // Ring is most up-to-date — dedupe by t+type+lib (heuristic)
  const seen = new Set();
  const out = [];
...
function aggregate(events, window) {
  ...
  // 4. lifecycle: job matched / not matched
  ...
  // 9. Clarification turn count (approximate — consecutive ambiguous per session/surface)
  // Simplified: show overall ambiguous vs clear count ratio; distribution is not yet implemented.
